...
export async function getAllMaterialForLecturer(): Promise<Material[]> {
...
export async function getAllMaterialsForStudent(courseId: number): Promise<Material[]> {
  const token = localStorage.getItem("token");

  const res = await fetch(`${COURSE_BASE_URL}/student/materials/${courseId}`, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`,
    },
  ...
  if (!res.ok) {
    ...
    throw new Error(errorData.error || "Gagal mengambil data material");
  }
...
export async function viewMateriFileStudent(id: number): Promise<{ blob: Blob; contentType: string }> {
  const token = localStorage.getItem("token");

  const res = await fetch(`${COURSE_BASE_URL}/student/materials/view/${id}`, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`,
    },
